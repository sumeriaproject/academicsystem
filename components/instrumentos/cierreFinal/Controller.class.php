...
class FuncioncierreFinal{
	...
	function action(){
        ...
        $this->procesarCierre($_REQUEST);
				...
				$this->context->render("cierreFinal",$variable);*/
  ...
  function procesarCierre($variable){
    ...
    if(isset($cursosCerrados[$variable["curso"]]) && !empty($cursosCerrados[$variable["curso"]])) {
      $cadenaSql = $this->sql->get("borrarNotasCerradas",array($this->activeYear,$variable["curso"]));
      $cursosCerrados = $this->resource->execute($cadenaSql,"");
