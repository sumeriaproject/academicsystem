...
function actualizarNotaCriterio(formSaraData,obj){
	...
	$.ajax({
		...
		success: function(respuesta) {
			...
			if(typeof respuesta.error=="undefined" || respuesta.error==""){
				obj.css("background-color","#E1F5A9");
				$("#not_final_"+estudiante).html(respuesta.notaFinal.toFixed(2));
				if(respuesta.notaFinalPorcentaje!=""){
...
function actualizarNotaCualitativa(formSaraData,obj){
	let nota = obj.val();
	estudiante = obj.attr('class');
	$.ajax({
		type: 'GET',
