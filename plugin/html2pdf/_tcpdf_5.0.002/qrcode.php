...
if (!class_exists('QRcode', false)) {
	...
	class QRcode {
		...
		/**
		 * @var Reed-Solomon components
		 * @access protected
		 */
		protected $rscomponents = array(); //of RSblock
		...
		protected function encodeMask($mask) {
			...
			$this->components = $this->rsBlockNum($spec);
			$ret = $this->init($spec);
			if ($ret < 0) {
				return NULL;
			}
		...
		protected function init($spec) {
			...
			for ($i=0; $i < $endfor; ++$i) {
				...
				$this->rscomponents[$blockNo] = array();
				$this->rscomponents[$blockNo]['dataLength'] = $dl;
				$this->rscomponents[$blockNo]['data'] = array_slice($this->datacode, $dataPos);
				$this->rscomponents[$blockNo]['eccLength'] = $el;
				$ecc = $this->encode_rs_char($rs, $this->rscomponents[$blockNo]['data'], $ecc);
				$this->rscomponents[$blockNo]['ecc'] = $ecc;
		 ...
		 * @return array rscomponents
		 */
		protected function getCode() {
			if ($this->count < $this->dataLength) {
				$row = $this->count % $this->components;
				$col = $this->count / $this->components;
				if ($col >= $this->rscomponents[0]['dataLength']) {
					$row += $this->b1;
				}
				$ret = $this->rscomponents[$row]['data'][$col];
			} elseif ($this->count < $this->dataLength + $this->eccLength) {
				$row = ($this->count - $this->dataLength) % $this->components;
				$col = ($this->count - $this->dataLength) / $this->components;
				$ret = $this->rscomponents[$row]['ecc'][$col];
			} else {
				return 0;
			}
			$this->count++;
		 ...
		 * @param int $version version
		 * @param int $level error correction level
		 * @param array $spec an array of ECC specification contains as following: {# of type1 components, # of data code, # of ecc code, # of type2 components, # of data code}
		 * @return array spec
		 */
